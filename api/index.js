// backend/api/index.js  ← apenas configura e exporta o Express app
...
// ── Rotas de saúde ────────────────────────────────────────────────────────────
app.get('/', (req, res) => res.json({ success: true, message: 'API HoraExtra' }));
...
app.get('/api/route', async (req, res) => {
  ...
  try {
    ...
    const url =
      `https://router.project-osrm.org/route/v1/driving/${fLng},${fLat};${tLng},${tLat}` +
      `?overview=full&geometries=polyline`;

    const data = await new Promise((resolve, reject) => {
      https
        .get(url, { timeout: 8000 }, (resp) => {
          let body = '';
    ...
    // Fallback: linha recta interpolada
    const R = 6371;
